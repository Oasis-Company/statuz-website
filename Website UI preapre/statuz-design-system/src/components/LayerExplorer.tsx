...
import { Layer, ThemeConfig } from '../types';
import { LAYERS } from '../data';
import { HelpCircle, Layers, CheckCircle2, ShieldAlert, GitCommit, Compass, Sparkles } from 'lucide-react';

interface Props {
  themeConfig: ThemeConfig;
}

export default function LayerExplorer({ themeConfig }: Props) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-5 flex flex-col gap-3">
        ...
        <div className="relative">
          {/* Continuous vertical line representing logo continuity */}
          ...
          {LAYERS.map((layer) => {
            ...
            return (
              <button
                ...
                style={{
                  borderRadius: themeConfig.radius === 'none' ? '0' : themeConfig.radius === 'sm' ? '0.275rem' : '0.5rem',
                }}
              >
                {/* Active selector continuous line indicator on left */}
                {isSelected && (
                  <div
                    className="absolute left-0 top-0 bottom-0 w-1"
                    style={{ backgroundColor: themeConfig.highlightColor }}
                  />
                )}
                
                {/* Custom circular indicator showing state of hierarchy */}
                <div
                  ...
                  style={{
                    borderColor: isSelected ? themeConfig.highlightColor : 'rgba(0,0,0,0.06)',
                  }}
                >
                  {getLayerIcon(layer.id, isSelected ? themeConfig.highlightColor : '#a1a1aa')}
                </div>
      ...
      {/* Layer detail viewer (Right Columns) */}
      <div className="lg:col-span-7">
        <div
          className="border border-zinc-200 bg-white p-6 md:p-8 flex flex-col relative transition-all shadow-sm"
          style={{
            borderRadius: themeConfig.radius === 'none' ? '0' : themeConfig.radius === 'sm' ? '0.375rem' : '0.85rem',
          }}
        >
          ...
          <div className="bg-zinc-50 p-4 border-l-2 border-zinc-900 mb-6 flex align-start gap-3">
            ...
            <div>
              <p className="text-[11px] font-mono uppercase tracking-wider text-zinc-400">Core Diagnostic Question Answered:</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            {/* Primitives */}
            <div>
              <p className="text-xs font-mono text-zinc-400 mb-2 uppercase tracking-wider">Semantic Declarations:</p>
              ...
              <div className="mt-4 pt-3 border-t border-zinc-150/60">
                <p className="text-[10px] text-zinc-400 leading-relaxed font-sans">
                  These primitive tokens comprise the structured API/YAML contract used by the toolchain.
                </p>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-mono text-zinc-400 uppercase tracking-wider">Schema Syntax preview</span>
                <span className="inline-block w-2.5 h-2.5 rounded-full bg-zinc-200" style={{ backgroundColor: themeConfig.highlightColor }} />
              </div>
