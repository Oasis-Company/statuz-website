...
import { CliCommand, ThemeConfig } from '../types';
import { CLI_COMMANDS } from '../data';
import { Terminal, Play, Clipboard, Check, RefreshCw } from 'lucide-react';

interface Props {
  themeConfig: ThemeConfig;
}

export default function CommandTerminal({ themeConfig }: Props) {
  const [activeCmdIdx, setActiveCmdIdx] = useState<number>(0);
  const activeCommand = CLI_COMMANDS[activeCmdIdx];

  const [simLines, setSimLines] = useState<string[]>([]);
  const [isTyping, setIsTyping] = useState<boolean>(false);
  const [copied, setCopied] = useState<boolean>(false);

  // Run simulation typing lines sequentially
  const runSimulation = () => {
    setIsTyping(true);
    setSimLines([]);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
      ...
      <div className="lg:col-span-4 flex flex-col gap-2.5 justify-between">
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] text-zinc-400 font-light">
                  {cmd.name === 'statuz resume' ? 'Layer 1 Core' : cmd.name === 'statuz validate' ? 'Drift Check' : 'Protocol'}
                </span>
              </button>
            );
          })}
        ...
        {/* Selected Command Description card */}
        <div 
        ...
        >
          <h4 className="text-xs font-mono text-zinc-400 uppercase tracking-widest mb-1.5 flex items-center justify-between">
            ...
            <span className="text-[10px] text-emerald-600 font-medium">NPM Published</span>
          </h4>
          ...
          <div className="space-y-1.5 pt-2.5 border-t border-zinc-200/60">
            <span className="text-[10.5px] font-mono text-zinc-400 block uppercase">Supported flags:</span>
            <div className="flex flex-wrap gap-1.5">
              {activeCommand.args.map((arg) => (
                <span key={arg} className="px-1.5 py-0.5 bg-zinc-200/60 font-mono text-[10px] text-zinc-800 rounded-none border border-zinc-200">
                  {arg}
                </span>
              ))}
            </div>
      ...
      {/* Terminal Display Simulator (Right) */}
        ...
        <div className="flex-1 bg-zinc-950 p-5 font-mono text-xs text-zinc-150 leading-relaxed border-x border-b border-zinc-800 rounded-b-lg overflow-y-auto max-h-[380px] flex flex-col justify-between shadow-inner">
          <div className="space-y-1.5 select-text">
            ...
            <div className="flex items-center gap-2 text-zinc-400 mb-3 border-b border-zinc-900 pb-2">
              ...
              <span className="text-zinc-50 font-semibold">{activeCommand.name}</span>
              {!isTyping && <span className="inline-block w-1.5 h-4 bg-zinc-400 animate-pulse ml-0.5" />}
            </div>

            {/* Render dynamically populated lines */}
            {simLines.map((line, idx) => {
              if (!line) return null;
              let lineClass = 'text-zinc-300';
              if (line.includes('✔') || line.includes('Created') || line.includes('Generated') || line.includes('successfully')) {
                lineClass = 'text-emerald-400 font-medium';
              } else if (line.includes('✖') || line.includes('Violation') || line.includes('Alarm') || line.includes('🚨')) {
              ...
              } else if (line.includes('Who:') || line.includes('Current Goal:') || line.includes('Completed Progress:') || line.includes('Checkpoint:') || line.includes('Next Step:')) {
              ...
              return (
                <div key={idx} className={`${lineClass} transition-all duration-300 transform translate-x-0`}>
                  {line}
                </div>
              );
            })}
